...
@section('content')
<div class="mb-8">
    <h1 class="text-3xl font-bold text-gray-800">تعديل المجتمع: {{ $community->name }}</h1>
    <p class="text-gray-600 mt-2">تعديل بيانات المجتمع والأجهزة المخصصة</p>
</div>

<div class="bg-white rounded-lg shadow-md p-6">
    <form action="{{ route('super-admin.communities.update', $community) }}" method="POST">
        @csrf
        @method('PUT')
        
        <div class="mb-6">
            <label for="name" class="block text-sm font-medium text-gray-700 mb-2">اسم المجتمع *</label>
            <input type="text" name="name" id="name" value="{{ old('name', $community->name) }}" required
        ...
        <div class="mb-6">
            ...
            <textarea name="description" id="description" rows="4"
                class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent @error('description') border-red-500 @enderror">{{ old('description', $community->description) }}</textarea>
            @error('description')
            <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
            @enderror
        </div>
        ...
        <div class="mb-6">
            <label class="flex items-center">
                <input type="checkbox" name="is_active" value="1" {{ old('is_active', $community->is_active) ? 'checked' : '' }}
                    class="ml-2 h-4 w-4 text-blue-600 focus:ring-blue-500 border-gray-300 rounded">
                <span class="text-sm font-medium text-gray-700">المجتمع نشط</span>
            </label>
        </div>
